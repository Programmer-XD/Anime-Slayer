# 𝗔𝗡𝗜𝗠𝚵 | 𝗦𝗟𝗔𝗬𝚵𝗥

Anime Slayer is an anime-themed RPG Discord bot where users battle and slay powerful anime characters, level up, collect gear, and rise through the ranks.

---

## 🚀 Features
- ⚔️ Battle & slay anime characters  
- 📈 Leveling & progression system  
- 🧰 Weapons, armor & inventory  
- 🎲 Spins & rewards  
- 🏆 Competitive gameplay & grinding  
- 🌌 Anime-inspired UI & mechanics  

---

## 🔐 Data & Privacy
Anime Slayer stores minimal data required for gameplay:
- Discord User ID  
- Game progress (levels, stats, inventory)

No personal data is sold or shared.

---

## 📜 Terms & Privacy
- **Terms of Service:**  
  https://Programmer-XD.github.io/AnimeSlayer 
- **Privacy Policy:**  
  https://Programmer-XD.github.io/AnimeSlayer 

---

## 🤖 Discord Policies
Anime Slayer complies with:
- Discord Terms of Service  
- Discord Developer Policies  

---

## ⚠️ Disclaimer
Anime Slayer is a fan-made project created for entertainment purposes.  
It is **not affiliated with Discord or any anime studios**.

---

## 🛠️ Support
For support, suggestions, or bug reports:
- Join the support server  
- Contact the bot developer  

---

© 2025 𝗔𝗡𝗜𝗠𝚵 | 𝗦𝗟𝗔𝗬𝚵𝗥
